...
/**
 * Checkbox: same script as ToggleButton (load, Tab to focus, Space, Enter, pointer click). The
 ...
 */
const e = entry('Checkbox')
const records: TargetRecord[] = []

for (const target of targetsFor(e)) {
  test(`Checkbox on ${target.name}`, async ({ page }) => {
    await page.goto(target.url)
    const steps: StepRecord[] = []
    const record = recorder(page, target, steps)
    await record('load')
    const tabsToFocus = await tabUntilFocused(page, target, 'Subscribe', record)
    await page.keyboard.press('Space')
    await record('space')
    await page.keyboard.press('Enter')
    await record('enter')
    await clickByTag(page, target, 'cb')
    await record('click')
